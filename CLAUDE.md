...
- **Mobile (Capacitor):** phased rollout plan in `docs/mobile/IMPLEMENTATION-PLAN.md` (Faz 0 baseline → Faz 5 closed beta), with companion docs in `docs/mobile/` (`API-CONTRACT.md`, `TEST-STRATEGY.md`, `SECURITY-PRIVACY.md`, `RISK-REGISTER.md`). Transport is HTTPS-only for store submission: `capacitor.config.ts` sets `cleartext: false` with `allowNavigation` scoped to `paydefteri.com`, the Android manifest carries no `usesCleartextTraffic`, `network_security_config.xml` permits cleartext for loopback addresses only, and iOS has no ATS exception. Keep it that way — reach a local API through `localhost`/`10.0.2.2`, not by reopening cleartext.
