...
public sealed class ImportInvitePlanAbidikGubidikTests
{
    ...
    [Fact]
    public async Task Positive_invite_accept_with_matching_email_then_second_accept_is_idempotent()
    {
        var owner = NewClient();
        await owner.RegisterAndLoginAsync();
        var (plan, partners) = await PlanTestHelper.CreatePlanWithPartnersAsync(owner);
        ...
        // Second accept by the same user is idempotent (retry / double-submit safe).
        var (again, joinedAgain) = await invitee.PostAsync<PlanDto>($"/api/invites/{invite.Token}/accept", new { });
        again.StatusCode.Should().Be(HttpStatusCode.OK);
        joinedAgain!.Id.Should().Be(plan.Id);
    }
