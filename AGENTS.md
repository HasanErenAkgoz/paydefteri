...
PayDefteri is an Angular 19 frontend and ASP.NET Core 8 API for shared installment and expense tracking. Keep business rules in `src/api/PayDefteri.Domain`, application commands/queries in `Application`, EF Core and external services in `Infrastructure`, and HTTP controllers/composition in `Api`. The Angular app is in `src/web/src/app`; place feature UI under `features/`, reusable UI under `shared/`, and API models/services under `core/`. Tests mirror the backend split in `tests/PayDefteri.Domain.Tests` and `tests/PayDefteri.Api.Tests`. Static web assets belong in `src/web/public`.

## Build, Test, and Development Commands

- `docker compose up -d` starts local PostgreSQL (`localhost:5432`).
- `dotnet run --project src/api/PayDefteri.Api` starts the API on `http://localhost:5096` (Swagger: `/swagger`).
- `npm start --prefix src/web` starts Angular on `http://localhost:4200`.
- `dotnet build PayDefteri.sln` builds all .NET projects.
...
- Add migrations with `dotnet ef migrations add Name -p src/api/PayDefteri.Infrastructure -s src/api/PayDefteri.Api -o Persistence/Migrations`.
