...
import { formatDateTr, formatTry } from '../../shared/utils/format';
...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, CurrencyTryPipe, DecimalPipe, RouterLink],
...
export class DashboardComponent implements OnInit {
  ...
  reload(): void {
    this.loading.set(true);
      ...
      next: (dto) => {
        ...
        this.planContext.setPlan(dto.planId, dto.title, dto.description);
  ...
  goEditInstallment(inst: DashboardInstallmentDto): void {
    void this.router.navigate(['/plans', this.planId, 'setup'], {
      queryParams: { edit: inst.id },
    });
  }
